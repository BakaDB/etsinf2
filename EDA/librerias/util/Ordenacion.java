...
public class Ordenacion {
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v, int i, int f) {
        ...
        T[] temp;        
        if (i == f) {
            temp = (T[]) new Comparable[] {v[i]};
        }
        //Base case: Array size 2
        else if (i+1 == f) {
            if (v[i].compareTo(v[f]) < 0)
                temp = (T[]) new Comparable[] {v[i], v[f]};
            else
                temp = (T[]) new Comparable[] {v[f], v[i]};
        }
        //Recursion
        else {
            int subArraySize = (i + f) / 2;
            T[] aux1 = mergeSort2(v, i, subArraySize);
            T[] aux2 = mergeSort2(v, subArraySize + 1, f);
            temp = merge2(aux1, aux2);
        }        
        return temp;
        */
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        ...
        T[] temp = (T[]) new Comparable[v1.length + v2.length];
        int i = 0, j = 0, k = 0;
        while (j < v1.length && k < v2.length) {
            if (v1[j].compareTo(v2[k]) < 0) temp[i++] = v1[j++];
            else temp[i++] = v2[k++];
        }
        
        while (j < v1.length) temp[i++] = v1[j++];
        while (k < v2.length) temp[i++] = v2[k++];
